...
- **Create job** `POST /jobs` `{ conversion:[{category?,target,options?}], input?:[…], process:bool,
  callback?, notify_status? }` → response includes `id`, per-job `token`, per-job upload `server`,
  and `status.code`.
...
- **Poll**: `GET /jobs/{id}` → terminal when `status.code ∈ {completed, failed}` (non-terminal:
  `created`, `incomplete`, `downloading`, `queued`, `processing`, and any unknown code).
...
- `convert(input, to, options?, {category?, timeout?, outputIndex?, filename?}) → ConversionResult`
  — create → (upload | remote input) → start → **poll to completion** → return. `to` is the target
  format string; `options` is the conversion-options map (passed 1:1 to the API's conversion
  `options`); the remaining controls are optional named/keyword arguments (never mixed into the
  options map, so open-ended API options can't collide with SDK keys). `input` is a local path, a
  URL (`^https?://`), or a stream. A URL is sent as a single started job with a `remote` input;
  anything else is staged, uploaded, then started.
- `convertAsync(input, to, options?, {callback?, category?, filename?}) → Job` — same, but returns
  once started without polling; sets `notify_status: true` when a `callback` is given.
- `download(output) → FileDownload`.
...
- `wait(id, timeoutSeconds?, throwOnFailure=true)` — poll with backoff until terminal; raise
  `ConversionFailedException` on `failed` (unless disabled), `TimeoutException` past the deadline.
...
- **Retries**: automatically retry 429, 5xx and network errors with capped exponential backoff,
  honoring `Retry-After`; surface the failure as a typed exception once retries are exhausted.
